...
"""
Model:
    UI segmentation and element detection implemented in AIM.
    Note: instead of paddleOCR, Tesseract is used for text recognition.
...
Description:
    ...
    models to perform classifications and text detection. This implementation is based on
    the UIED-image segmentation method used in AIM (https://interfacemetrics.aalto.fi).


References:
    1. A.Oulasvirta, S. D. Pascale, J. Koch, T. Langerak, J. Jokinen, K. Todi,
       M. Laine, M. Kristhombuge, Y. Zhu, A. Miniukovich, G. Palmas, and T. Weinkauf. (2018).
       Aalto Interface Metrics (AIM): A Service and Codebase for Computational GUI Evaluation.
       In Proceedings of the 31st Annual ACM Symposium on User Interface Software and
       Technology Adjunct Proceedings (UIST '18 Adjunct).
       ACM, New York, NY, USA, 16-19. DOI: 10.1145/3266037.3266087

    2.  Xie, M., Feng, S., Xing, Z., Chen, J., and Chen, C. (2020). UIED: A
    ...
    3.  Chen, J., Xie, M., Xing, Z., Chen, C., Xu, X., Zhu, L., and Li, G.
...
# First-party modules
from image_preprocessing.segmentation.image_seg_utils import (
    Component,
    binarization,
...
from image_preprocessing.segmentation.merge_seg_utils import (
    Element,
    check_containment,
...
from image_preprocessing.segmentation.text_seg_utils import (
    Text,
    text2json,
    text_cvt_orc_format_tesseract,
    visualize_texts,
...
class Segmentation:
    ...
    # Private constants
    _IS_CLF: bool = True  # use cnn classifier to determine type of components
    ...
    def text_detection(
        ...
        """
          ...
          :param show:
        """
        # Convert the image and perform OCR using Tesseract
        texts: List[Text] = text_cvt_orc_format_tesseract(Image.fromarray(img))
        texts: List[Text] = merge_intersected_texts(texts)
        texts: List[Text] = text_filter_noise(texts)
        texts: List[Text] = text_sentences_recognition(texts)

        if show or write_path is not None:
            # Visualize the detected texts on the image
            visualize_texts(img, texts, color=(0, 0, 255), line=2, show=show, write_path=write_path)

        # Convert the detected texts to a structured JSON format
    ...
    def execute(
        ...
        # Get NumPy array
        img_nparray: np.ndarray = np.array(gui_image)

        # Convert NumPy array to Cv2 format (BGR)
        img_cv: np.ndarray = cv2.cvtColor(img_nparray, cv2.COLOR_RGB2BGR)

        key_params: Dict = cls._KEY_PARAMS_DESKTOP
        ...
        board_bgr, components = cls.merge(img_cv, compo_json, text_json, key_params)

        # Convert board to pillow and then to bytesIO
        board_rgb: np.ndarray = cv2.cvtColor(board_bgr, cv2.COLOR_BGR2RGB)
        board_im: Image.Image = Image.fromarray(board_rgb)
        board_bytes_io = BytesIO()
        board_im.save(board_bytes_io, format='PNG')
        board_bytes_io.seek(0)

        # Save in component dictionary
        components["img_bytes_io"] = board_bytes_io
