...
def draw_boundary(components, shape, show=False):
    """
    Draw boundary of objects on the black white
        ...
        for n, border in enumerate(self.boundary):
            ...
            for i in range(int(3 + len(border) * 0.02), len(border) - 1):
                ...
                if abs(depth) / adj_side > 0.3:
                    ...
                    # if the abnm is too big, the shape should not backend a rectangle
            ...
            # if the pit is too big, the shape should not backend a rectangle
...
def rm_line(
    ...
    for i, row in enumerate(binary):
        ...
        if is_valid_line(row):
            ...
            # end the line
            if check_line:
                # thin enough to backend a line, then start checking gap
...
def component_detection(
    ...
    :param min_rec_evenness: If not pass then this object cannot backend rectangular
    :param max_dent_ratio: If not pass then this object cannot backend rectangular
        ...
        for y in range(0, column, step_v):
            if mask[x, y] == 0:
                ...
                if compo.compo_is_line(line_thickness):
                    continue
                # ignore non-rectangle as blocks must backend rectangular
